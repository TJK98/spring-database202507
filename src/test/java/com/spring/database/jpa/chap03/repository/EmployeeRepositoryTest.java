...
class EmployeeRepositoryTest {
    ...
    void insertBulk() {
        ...
        /*
            JPA는 영속성 컨텍스트라는 대형 데이터 창고를 운영하여 메모리에 데이터가 존재하면
            굳이 DB로 가서 조회하지 않고 컨텍스트 창고 안에 있는 데이터를 재활용함.

            아래의 코드는 데이터 창고(영속성 컨텍스트)를 지워버리는 코드
         */
        em.flush();
        em.clear();
    }
    ...
    void findTest() {
        ...
        // 명시적으로 부서 정보를 참조 - LAZY LOADING은 이 시점에 부서 정보를 가져 옵니다.
    ...
    void changeTest() {
        ...
        //when
        foundEmp.setDepartment(foundDept); // 사원 쪽에서 부서 정보 변경
        // 양방향 매핑에서는 반대편에서도 수동으로 변경 처리가 진행 되어야 함.
