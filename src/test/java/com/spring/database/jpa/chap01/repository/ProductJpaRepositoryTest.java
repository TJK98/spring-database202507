...
//@DataJpaTest // @Repository 빈만 로딩, 롤백 옵션 자동 포함, 내장에 존재하는 DB만 사용 가능
class ProductJpaRepositoryTest {
    ...
    void findOneTest() {
        ...
        Long id = 3L;
        //when
//        Product foundProduct = productJpaRepository.findById(id).get();
        // NPE를 방지하기 위한 Optional으로 감싸면 된다.
        Optional<Product> foundProduct = productJpaRepository.findById(id);
        //then
        System.out.println("상품명 = " + foundProduct.map(Product::getName).orElse("상품이 존재하지 않습니다."));
    }
...
/*        foundProduct.setName(newName);
    ...
    @Test
    @DisplayName("상품을 전체 조회하면 총 4개의 상품이 조회된다.")
    void findAllTest() {
        //given
